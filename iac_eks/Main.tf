...
# Provider em arquivo separado?
provider "aws" {
  region = var.region
}

# Validação não necessária no primeiro momento, comentando código
...
module "vpc" {
  ...
  # Avaliar necessidade futuramente 
...
# Armazenamento de bloco (Necessidade futura?)
#  cluster_addons = {
#    aws-ebs-csi-driver = {
...
# Rever configurações do EBS posteriormente
